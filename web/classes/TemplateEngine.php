...
/**
 * Based on RainTPL 2.7.2 originally realized by Federico Ulfo.
...
class TemplateEngine
{
	...
	function draw( $tpl_name, $return_string = false ){
        ...
        // rain get the output buffer to save the output in the cache or to return it as string
            ...
            if( $this->cache )
                file_put_contents( $this->tpl['cache_filename'], "<?php if(!class_exists('TemplateEngine')){exit;}?>" . $raintpl_contents );
	...
	*/
	...
	protected function compileFile( $tpl_basename, $tpl_basedir, $tpl_filename, $cache_dir, $compiled_filename ){
		...
		//compile template
		$template_compiled = "<?php if(!class_exists('TemplateEngine')){exit;}?>" . $this->compileTemplate( $template_code, $tpl_basedir );
